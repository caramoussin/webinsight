<script lang="ts">
  ...
  const handleAddFeed = async (feedData: CreateFeed) => {
    ...
    try {
      await pipe(
        feedsStore.createFeed(feedData),
        E.provide(FetchHttpClient.layer),
        E.runPromise
      );
      showAddModal = false;
  ...
  const handleEditFeed = async (feedId: string, updateData: { name?: string; url?: string; collectionId?: string }) => {
    try {
      await pipe(
...
<div class="container mx-auto px-4 py-8">
  ...
  {:else if feedsStore.feeds.length === 0}
    <div class="text-center py-12">
      ...
      <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">No feeds yet</h3>
